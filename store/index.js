...
const store = () =>
	...
	getters: {
		...
		getUSDPriceToSymbol(state) {
			return (symbol) => {
				let elem = state.USDPrices.find(elem => elem.symbol === symbol) || {};
				return elem.USDPrice || 0;
			};
		},
		getPairs(state) {
			return state.pairs;
	...
	actions: {
		...
		getUSDPrices({ commit }) {
			this.$axios
				.$get('/api/data/getUSDPrices')
				.then(res => {
					if(res.status === 'ok') {
						commit('setUSDPrices', res.data);
					} else {
						console.log(res);
					}
				})
